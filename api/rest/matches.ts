...
// GET /api/rest/matches/categories
app.get("/categories", async (c) => {
  try {
    return c.json(CREATOR_CATEGORIES);
  } catch (error) {
    return handleError(c, error);
  }
});

// POST /api/rest/matches/requirements
app.post("/requirements", async (c) => {
  try {
    ...
    return c.json({ success: true, requirementId: Number(result[0].insertId) });
  } catch (error) {
    return handleError(c, error);
  }
});

// GET /api/rest/matches/my-requirements
app.get("/my-requirements", async (c) => {
  try {
    const user = requireRole(requireAuth(await getUser(c.req.raw)), "brand");
    const db = getDb();
    const reqs = await db.select().from(brandRequirements).where(eq(brandRequirements.brandId, user.id)).orderBy(desc(brandRequirements.createdAt));
    return c.json(reqs);
  } catch (error) {
    return handleError(c, error);
  }
...
// POST /api/rest/matches/run-matching
app.post("/run-matching", async (c) => {
  try {
    const user = requireRole(requireAuth(await getUser(c.req.raw)), "brand");
    const input = z.object({ requirementId: z.number() }).parse(await c.req.json());
    const db = getDb();

    const requirement = await db.query.brandRequirements.findFirst({ where: eq(brandRequirements.id, input.requirementId) });
    if (!requirement || requirement.brandId !== user.id) {
...
// GET /api/rest/matches/requirements/:id/matches
app.get("/requirements/:id/matches", async (c) => {
  try {
    const user = requireRole(requireAuth(await getUser(c.req.raw)), "brand");
    const requirementId = z.coerce.number().parse(c.req.param("id"));
    const db = getDb();

    const requirement = await db.query.brandRequirements.findFirst({ where: eq(brandRequirements.id, requirementId) });
    if (!requirement || requirement.brandId !== user.id) {
      return c.json([]);
    }

    const matches = await db.select().from(aiMatches).where(eq(aiMatches.requirementId, requirementId)).orderBy(desc(aiMatches.matchScore));
...
// GET /api/rest/matches/my-matches
app.get("/my-matches", async (c) => {
  try {
    const user = requireRole(requireAuth(await getUser(c.req.raw)), "influencer");
    const db = getDb();
    const matches = await db.select().from(aiMatches).where(eq(aiMatches.influencerId, user.id)).orderBy(desc(aiMatches.matchScore));
    const enriched = await Promise.all(matches.map(async (m) => {
...
// POST /api/rest/matches/:id/status
app.post("/:id/status", async (c) => {
  try {
    const user = requireAuth(await getUser(c.req.raw));
    const matchId = z.coerce.number().parse(c.req.param("id"));
    const input = z.object({ status: z.enum(["suggested", "invited", "accepted", "declined"]) }).parse(await c.req.json());
    const db = getDb();
    await db.update(aiMatches).set({ status: input.status }).where(eq(aiMatches.id, matchId));
...
// GET /api/rest/matches/insights
app.get("/insights", async (c) => {
  try {
    requireAuth(await getUser(c.req.raw));
    const db = getDb();
