...
// GET /api/rest/admin/overview
app.get("/overview", async (c) => {
  try {
    requireRole(requireAuth(await getUser(c.req.raw)), "admin");
    const db = getDb();
...
// GET /api/rest/admin/monthly-growth
app.get("/monthly-growth", async (c) => {
  try {
    requireRole(requireAuth(await getUser(c.req.raw)), "admin");
    const db = getDb();
    const months: string[] = [];
    ...
    return c.json({ monthlyData, userGrowth, revenueGrowth });
  } catch (error) {
    return handleError(c, error);
  }
});
...
app.get("/users", async (c) => {
  try {
    ...
    const conditions = query.role ? eq(users.role, query.role as any) : undefined;
...
app.get("/payments", async (c) => {
  try {
    ...
    const conditions = query.status ? eq(payments.status, query.status as any) : undefined;
...
app.get("/campaigns", async (c) => {
  try {
    ...
    const conditions = query.status ? eq(campaigns.status, query.status as any) : undefined;
    const result = conditions
...
// PUT /api/rest/admin/users/:id/role
app.put("/users/:id/role", async (c) => {
  try {
    requireRole(requireAuth(await getUser(c.req.raw)), "admin");
    const userId = z.coerce.number().parse(c.req.param("id"));
    const input = z.object({
      role: z.enum(["user", "brand", "influencer", "admin"]),
    }).parse(await c.req.json());

    const db = getDb();
    await db.update(users).set({ role: input.role, updatedAt: new Date() }).where(eq(users.id, userId));
...
// POST /api/rest/admin/campaigns/:id/moderate
app.post("/campaigns/:id/moderate", async (c) => {
  try {
    requireRole(requireAuth(await getUser(c.req.raw)), "admin");
    const campaignId = z.coerce.number().parse(c.req.param("id"));
    const input = z.object({
...
// GET /api/rest/admin/subscriptions
app.get("/subscriptions", async (c) => {
  try {
    ...
    return c.json({ brandSubs: enrichedBrandSubs, creatorSubs: enrichedCreatorSubs });
  } catch (error) {
    return handleError(c, error);
  }
});

// POST /api/rest/admin/users/:id/downgrade
app.post("/users/:id/downgrade", async (c) => {
  try {
    requireRole(requireAuth(await getUser(c.req.raw)), "admin");
    const userId = z.coerce.number().parse(c.req.param("id"));
    const input = z.object({
    ...
    }).where(eq(idField as any, userId));
...
// GET /api/rest/admin/revenue-breakdown
app.get("/revenue-breakdown", async (c) => {
  try {
    ...
    return c.json({ byType, monthlyRevenue });
  } catch (error) {
    return handleError(c, error);
  }
});

// GET /api/rest/admin/top-creators
...
// GET /api/rest/admin/recent-activity
app.get("/recent-activity", async (c) => {
  try {
    ...
    return c.json({ recentUsers, recentPayments, recentCampaigns });
  } catch (error) {
    return handleError(c, error);
  }
});
