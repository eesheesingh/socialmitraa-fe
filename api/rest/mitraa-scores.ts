...
function calculateScoreComponents(profile: any, applications: any[], paymentsData: any[]): ScoreComponents {
  ...
  if (avgLikes > 0 && avgComments > 0) {
    ...
    if (ratio > 200) engagementAuthenticity -= 20;
...
// POST /api/rest/mitraa-scores/calculate
app.post("/calculate", async (c) => {
  try {
    ...
    return c.json({ overallScore, category, components, scoreChange });
  } catch (error) {
    return handleError(c, error);
  }
});

// GET /api/rest/mitraa-scores/me
app.get("/me", async (c) => {
  try {
    const user = requireAuth(await getUser(c.req.raw));
    requireRole(user, "influencer");
    ...
    const [score] = await db
      .select()
      .from(creditScores)
      .where(eq(creditScores.creatorId, user.id));
...
// GET /api/rest/mitraa-scores/admin/all
app.get("/admin/all", async (c) => {
  try {
    const user = requireAuth(await getUser(c.req.raw));
    requireRole(user, "admin");
    const db = getDb();
    const scores = await db.select().from(creditScores).orderBy(desc(creditScores.overallScore));
...
// GET /api/rest/mitraa-scores/:creatorId/report
app.get("/:creatorId/report", async (c) => {
  try {
    const user = requireAuth(await getUser(c.req.raw));
    requireRole(user, "brand");
    const creatorId = z.coerce.number().parse(c.req.param("creatorId"));
    const db = getDb();
    const [score] = await db
      .select()
      .from(creditScores)
    ...
    if (!score) return c.json({ error: "No Mitraa Score available for this creator" });
...
// GET /api/rest/mitraa-scores/:creatorId
app.get("/:creatorId", async (c) => {
  try {
    const creatorId = z.coerce.number().parse(c.req.param("creatorId"));
    const db = getDb();
    const [score] = await db
